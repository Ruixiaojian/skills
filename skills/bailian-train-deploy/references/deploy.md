...
> 本文件补充 [`SKILL.md`](../SKILL.md) 第 5 步「创建部署」的 `--plan` 选项、`--model` 歧义与计费/运维细则。流程编排与避坑见 SKILL.md。
...
| 模型来源 | 可用 plan | 说明 |
|---|---|---|
| 链路 A（微调输出，如 `qwen3-8b-ft-...`） | `lora`（默认）/ `mu` | `lora` 按 token 计费，适合验证/低负载 |
...
- `mu`：需 `--template-id` / `--capacity`（独占资源，闲置也计费）；省略时从 `bl deploy models --source base` 自动取。
