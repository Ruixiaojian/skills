...
| 实时 API 方案对比：Omni Realtime API vs Realtime API | `wiki/comparisons/realtime-api-comparison.md` |
| 多模态生成能力对比：图像生成、视频生成与3D生成 | `wiki/comparisons/generation-apis-comparison.md` |
| 长期记忆方案对比：Long Term Memory 与 Memory Library | `wiki/comparisons/memory-solutions-comparison.md` |
| 应用构建框架对比：Managed Agents、Application Component API 与 Toolkits and Frameworks | `wiki/comparisons/application-frameworks-comparison.md` |
| 模型部署方式对比：Model Deployment、Model Production 与 Fine-tuning | `wiki/comparisons/model-deployment-options.md` |
