...
不同模型按分档抵扣系数计费：视频生成模型单次调用消耗的 Credits 明显高于文本对话，且随生成视频的时长和分辨率上升，可能在短时间内占用较多的每 5 小时或每 7 天限额（任一层限额触顶即暂停服务）。建议首次使用时以较短时长、较低分辨率试跑，通过控制台订阅页用量详情确认单次实际消耗后，再决定后续用量。

此外，视频生成等异步任务的 Credits 在任务完成后统一结算，而非提交时立即扣除。如果短时间内提交了多个异步任务，所有任务完成后集中结算的 Credits 可能导致限额快速触顶。
