...
-   **多模态模型支持**：覆盖文本生成、推理、视觉理解、图片生成、语音合成、视频生成等能力。
...
-   **单次消耗高**：视频生成的 Credits 随时长和分辨率上升，可能在短时间内占用较多的每 5 小时或每 7 天限额。建议首次使用时以较短时长、较低分辨率试跑，通过控制台订阅页用量详情确认单次实际消耗后，再决定后续用量。
...
额度重置需消耗重置次数，请关注官方公告或活动。

## **支持的模型**

**重要**

个人版支持 qwen3.8-max 和 qwen3.8-max-preview 模型，享有以下限时权益：

-   **qwen3.8-max 限时夜间五折**：每晚 22:00 - 次日 08:00 期间调用 qwen3.8-max 模型，Credits 消耗享 5 折优惠。
    
-   **qwen3.8-max-preview 预览版**：qwen3.8-max-preview 当前为预览版本，**预览期间模型能力会持续迭代升级**。预览结束后该模型会下线或替换成正式版本。
    
-   **qwen3.8-max-preview 限时加量 10 倍**：限时活动期间，模型调用 Credits 消耗低至 1 折，相当于增加 10 倍用量。
    
-   **qwen3.8-max-preview 限时夜间折上折**：在现有 1 折优惠基础上，每晚 22:00 - 次日 08:00 期间调用模型，Credits 消耗再享 2 折（即原标准的 0.2 折）。
