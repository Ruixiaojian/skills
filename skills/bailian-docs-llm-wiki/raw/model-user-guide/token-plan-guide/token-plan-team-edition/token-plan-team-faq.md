...
团队版订阅归属于购买者账号。被分配席位的成员无法在自己账号下查看 Token Plan 订阅或用量明细——**用量分析为所有者功能**，需由购买者（所有者）在 Token Plan 控制台「我的订阅 → 用量分析」查看各成员的 Credits 消耗（详见团队管理）。添加席位时系统会为成员自动生成专属 API Key，成员使用该 Key 即可调用模型，无需在自己账号下查看订阅。
