...
首次开通阿里云百炼（需完成**实名认证**）后由系统自动发放，无需手动领取或购买任何产品。
...
该功能默认关闭。开启后，须在该模型免费额度完全耗尽后方可关闭该开关（免费额度分钟级出账，数据可能有延迟，若刚耗尽仍不可操作，请刷新或等待片刻）。可在免费额度页面操作列关闭单个模型的**免费额度用完即停**开关，或经批量操作下拉菜单选择**批量关闭**。
