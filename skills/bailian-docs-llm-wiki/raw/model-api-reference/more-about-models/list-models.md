...
示例：`providers=qwen&providers=deepseek`
...
-   `TG`：文本生成（Text Generation）
    
-   `Reasoning`：深度推理（Advanced Reasoning）
    
-   `VU`：视觉理解（Visual Understanding）
    
-   `IG`：图像生成（Image Generation）
    
-   `VG`：视频生成（Video Generation）
    
-   `ASR`：语音识别（Speech Recognition）
    
-   `TTS`：语音合成（Text-to-Speech）
    
-   `TR`：文本向量（Text Embedding）
    
-   `ME`：多模态向量（Multimodal Embedding）
    
-   `Multimodal-Omni`：全模态（Omni）
    
-   `Realtime-Omni`：实时全模态（Realtime Omni）
...
示例：`capabilities=TG&capabilities=Reasoning`
...
-   `international`：国际模式（除中国大陆外）
    
-   `asia-pacific-china`：中国大陆
