...
要查询的调优任务的ID。即[创建调优任务返回参数](https://help.aliyun.com/zh/model-studio/create-fine-tuning-job-api#5029d0724bzmm)中的job\_id。
...
Checkpoint 数组。按 LM epoch × FM epoch 的乘积从大到小排序（乘积越大代表 LM 与 FM 双端调优越充分）。
...
该 Checkpoint 对应的模型 ID，可作为创建部署接口的 `model_name` 入参。**仅在** `**status=SUCCEEDED**` **时返回**。
...
调优任务ID，可通过[创建训练任务](https://help.aliyun.com/zh/model-studio/create-fine-tuning-job-api#t6612754.html)或列举训练任务接口获取。
...
Checkpoint名称，可通过列举Checkpoint或列举验证产物接口获取。
...
该 Checkpoint 对应的模型 ID，可作为[创建部署](https://help.aliyun.com/zh/model-studio/create-deployment-api#t6612768.html)接口的 `model_name` 入参。**仅在** `**status=SUCCEEDED**` **时返回**。
