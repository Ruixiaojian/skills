...
- **知识问答**：支持流式知识问答（SSE），输出包含规划（planning）、工具调用（tool calling）和生成（generation）三阶段响应，可直接集成至对话界面。该接口能力在 [知识检索与问答 (raw/application-api-reference/knowledge.md)](../../raw/application-api-reference/knowledge.md) 中明确定义。
...
1. **构造 Base URL**：将业务空间 ID 替换为 `{workspaceId}`，例如 `https://my-workspace.cn-beijing.maas.aliyuncs.com`  
2. **发起检索请求**：
   ```bash
   curl -X POST "https://my-workspace.cn-beijing.maas.aliyuncs.com/api/v1/indices/knowledge/search" \
     -H "Authorization: Bearer sk-xxx" \
     -H "Content-Type: application/json" \
     -d '{"query":"如何重置密码？","top_k":3}'
   ```
3. **发起问答请求（流式）**：
   ```bash
   curl -X POST "https://my-workspace.cn-beijing.maas.aliyuncs.com/api/v2/apps/knowledge/chat" \
     -H "Authorization: Bearer sk-xxx" \
     -H "Content-Type: application/json" \
     -d '{"messages":[{"role":"user","content":"忘记密码怎么办？"}]}'
   ```

## 限制和注意事项

- **限流策略**：默认按用户维度限流 25 QPS，超限返回 `429 Too Many Requests`；无突发配额，不可降级绕过。
- **知识库状态依赖**：仅对 `ACTIVE` 状态的知识库生效；若知识库处于 `BUILDING` 或 `FAILED` 状态，检索/问答将失败且不降级至其他库。
- **输入长度**：`search` 接口 query 最长 2048 字符；`chat` 接口单次请求 messages 总 tokens 建议 ≤ 8192（含系统提示与历史上下文），超长可能截断或报错。
- **无异步轮询机制**：`/chat` 接口虽为流式，但不提供 `task_id` 或回调地址，客户端需自行处理连接中断与重连。
