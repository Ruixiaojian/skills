...
- **限流策略**：默认按用户维度限流 25 QPS，超限返回 `429 Too Many Requests`；无突发配额，需自行实现退避重试。
- **知识库状态依赖**：检索与问答仅对「已发布」且「状态为 active」的知识库生效；草稿或禁用状态的知识库不会参与计算。
- **无显式模型切换机制**：当前不支持通过参数指定 embedding 模型或 LLM，所有语义处理由平台自动调度，版本升级由服务端控制。
- **路径差异**：`/search` 属于 `v1/indices/knowledge/` 路径体系，而 `/chat` 属于 `v2/apps/knowledge/`，二者鉴权方式一致但路由层级不同，不可混用。
