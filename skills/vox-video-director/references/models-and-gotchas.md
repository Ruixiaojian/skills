...
## Bailian CLI backend (default)

The default `bailian_cli` provider invokes authenticated `bl` commands. It passes local
files directly, lets the CLI handle temporary DashScope uploads, and requests a local
output file for every result.

| Job | Command | Default model |
|---|---|---|
| Poster | `bl image generate` | `qwen-image-3.0` |
| Anchored poster | `bl image edit` | `qwen-image-3.0` |
| Poster animation | `bl video generate` | `happyhorse-1.1-i2v` |
| A-roll restyle | `bl video edit` | `happyhorse-1.0-video-edit` |
| Narration | `bl speech synthesize` | `cosyvoice-v3-flash` |
| Transcription | `bl speech recognize` | `fun-asr` |

- Run `bl auth status` before generation. Never add an API key to `beats.json`.
- A running CLI task is not auto-resubmitted on a stall, avoiding duplicate charges.
- `bl` 1.14.x has no music generator. Supply `bgm_path`, or assemble without BGM.
- Old Atlas model IDs are mapped to Bailian defaults by this provider. Bailian-native IDs
  are passed through unchanged.

The Atlas-specific notes below apply only to explicit `"provider": "atlas_cloud"` projects.
