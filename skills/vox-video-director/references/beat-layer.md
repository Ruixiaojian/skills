...
- **Beat-count & length (content-driven):**

  | Duration | Beats (posters) | Beat length | VO words |
  |---|---|---|---|
  | **30s** | 4–6 | 5–10s, mixed | ~70–80 |
  | **60s** | 7–10 | 5–10s, mixed | ~130–150 |

  Use `scripts/timing.py`: estimate from narration text, then replace the estimate with the
  measured TTS duration. Round up to 0.5s, add a short edit tail, clamp at 5–10s, and split
  any beat that would exceed 10s.
- **Proportions:** hook 1–3s → body 70–80% → payoff 10–20% → end/CTA 0–2s.
- **Change something visually every 5–10s.** For 8–10s segments, write richer element motion
  with a clear settle point so the living poster does not read as a static hold.
...
Coverage plays out **across beats** (one content-sized poster/clip per beat): move-in `EST_WIDE→MEDIUM→CLOSE`
(build intensity, peak on CLOSE) · move-out `CLOSE→…→WIDE` (reveal context; good ending) ·
Use an establishing-wide beat followed by a separate detail beat when the narration has two
clauses. This keeps both generated clips inside the 5–10s content-sized range.
...
BEATS: 30s→4–6 @5–10s · 60s→7–10 @5–10s · hook ≤3s · duration follows narration length
```
