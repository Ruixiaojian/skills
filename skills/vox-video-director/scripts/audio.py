#!/usr/bin/env python3
"""
Audio stage: per-beat narration (one consistent voice) plus optional BGM.

The default Bailian CLI backend uses CosyVoice through ``bl speech synthesize``.
Because bl 1.14.x does not expose a music-generation command, set ``bgm_path``
to an existing instrumental file or assemble without music.  The optional Atlas
backend retains its original generated-BGM behavior.
...
from timing import apply_content_timing
...
VOICE_MODEL = "xai/tts-v1"  # Bailian provider maps legacy IDs to cosyvoice-v3-flash
...
def run(project_dir: str):
    ...
    voice = doc.get("voice", {})
    default_voice = "leo" if prov.name == "atlas_cloud" else "longtian_v3"
    ...
    # Reuse an explicit/local BGM. Bailian CLI currently has no music generation
    # command, so its projects simply continue without BGM when none is supplied.
    bgm_path = doc.get("bgm_path") or os.path.join(adir, "bgm.mp3")
    if os.path.exists(bgm_path):
        doc["bgm_path"] = bgm_path
        print(f"[bgm] reuse existing {bgm_path}")
    elif prov.supports_music:
        ...
        print("[bgm] skipped: bl has no music-generation command; set bgm_path to add music")
    ...
    issues = apply_content_timing(doc, prefer_audio=True)
    if issues:
        print("[timing] split narration before image/video generation:")
        for issue in issues:
            print(" -", issue)
    else:
        print("[timing] content-driven segment durations updated from measured narration")
