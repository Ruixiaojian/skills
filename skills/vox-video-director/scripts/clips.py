...
from timing import apply_content_timing, resolve_beat_duration

VIDEO_MODEL = "google/gemini-omni-flash/image-to-video"  # mapped by Bailian provider
...
def run(project_dir, only=None):
    ...
    with open(bpath) as f:
        doc = json.load(f)
    timing_issues = apply_content_timing(doc, prefer_audio=True)
    if timing_issues:
        print("TIMING BLOCK: fix these items before starting billable video jobs:")
    ...
    for beat in doc["beats"]:
        for shot, key in shots_of(beat):
            ...
            fallback_dur, _ = resolve_beat_duration(beat, doc, prefer_audio=True)
            # Video APIs take whole seconds. Round UP so narration is never cut;
            # assembly trims the generated clip back to the exact half-second edit duration.
            dur = int(math.ceil(float(shot.get("dur", beat.get("dur", fallback_dur or 5)))))
            if "happyhorse" in model or "wan2" in model:
                params = dict(image=url, duration=dur, ratio=aspect,
                              resolution=vid_res, generate_audio=False)
            elif "seedance" in model:                 # ratio (not aspect_ratio); real-person OK
                params = dict(image=url, duration=dur, ratio=aspect,
                              resolution=vid_res, generate_audio=False)
