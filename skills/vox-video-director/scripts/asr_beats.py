...
def run(project_dir, source, language=None, keyterm=None, max_beat_dur=MAX_BEAT_DUR,
    ...
    print("preparing audio for ASR...")
    audio_url = prov.upload(audio_path)
    print(f"transcribing via {prov.name}...")
    result = prov.transcribe(audio_url, language=language,
    ...
    doc = {
        ...
        "provider": provider_name or "bailian_cli",
        "video_model": ("google/gemini-omni-flash/video-edit"
                        if prov.name == "atlas_cloud" else "happyhorse-1.0-video-edit"),
