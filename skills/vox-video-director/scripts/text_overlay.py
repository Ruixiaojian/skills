...
def _wrap(draw, text, fnt, max_w):
    # CJK text usually has no spaces; wrap by glyph instead of treating the
    # entire sentence as one unbreakable word.
    if " " not in text.strip() and any("\u3400" <= ch <= "\u9fff" for ch in text):
        lines, cur = [], ""
        for ch in text:
            trial = cur + ch
