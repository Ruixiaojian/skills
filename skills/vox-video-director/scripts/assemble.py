...
def run(project_dir):
    with open(os.path.join(project_dir, "beats.json")) as f:
        doc = json.load(f)
    beats = doc["beats"]
    timing_mode = doc.get("timing_mode", doc.get("timing", {}).get("mode", "content"))
    if timing_mode == "content" and doc.get("timing_issues"):
        raise ValueError("Resolve timing_issues in beats.json before assembly")
    ...
    if has_bgm:
        ...
        chain.append(f"[narrmix]volume=1.4,atrim=0:{total}[a]")
    filt = ";".join(chain)
